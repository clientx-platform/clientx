class Integrations::Csml::ProcessorService < Integrations::BotProcessorService
  ...
  def client_params(session_id)
    {
      bot_id: "clientx-bot-#{conversation.inbox.id}",
      channel_id: "clientx-bot-inbox-#{conversation.inbox.id}",
      user_id: session_id
    }
  end
  ...
  def bot_payload
    {
      id: "clientx-csml-bot-#{agent_bot.id}",
      name: "clientx-csml-bot-#{agent_bot.id}",
      default_flow: 'clientx.bot_flow',
      flows: [
        {
          id: "clientx-csml-bot-flow-#{agent_bot.id}-inbox-#{conversation.inbox.id}",
          name: 'clientx.bot_flow',
