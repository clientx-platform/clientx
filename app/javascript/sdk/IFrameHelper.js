...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'clientx.live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$clientx.position}`;
    if (window.$clientx.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$clientx.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('clientx.live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `clientx-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (
        ...
        e.data.indexOf('clientx-widget:') !== 0
      ) {
        return;
      }
      const message = JSON.parse(e.data.replace('clientx-widget:', ''));
  ...
  events: {
    loaded: message => {
      updateAuthCookie(message.config.authToken, window.$clientx.baseDomain);
      window.$clientx.hasLoaded = true;
      const campaignsSnoozedTill = Cookies.get('cw_snooze_campaigns_till');
      IFrameHelper.sendMessage('config-set', {
        locale: window.$clientx.locale,
        position: window.$clientx.position,
        hideMessageBubble: window.$clientx.hideMessageBubble,
        showPopoutButton: window.$clientx.showPopoutButton,
        widgetStyle: window.$clientx.widgetStyle,
        darkMode: window.$clientx.darkMode,
      ...
      if (window.$clientx.user) {
        IFrameHelper.sendMessage('set-user', window.$clientx.user);
      }
      ...
      if (!window.$clientx.resetTriggered) {
        dispatchWindowEvent({ eventName: CLIENTX_READY });
      }
    },
    error: ({ errorType, data }) => {
      dispatchWindowEvent({ eventName: CLIENTX_ERROR, data: data });
    ...
    onMessage({ data }) {
      dispatchWindowEvent({ eventName: CLIENTX_ON_MESSAGE, data });
    },
    setBubbleLabel(message) {
      setBubbleText(window.$clientx.launcherTitle || message.label);
    },

    setAuthCookie({ data: { widgetAuthToken } }) {
      updateAuthCookie(widgetAuthToken, window.$clientx.baseDomain);
    },

    setCampaignReadOn() {
      updateCampaignReadStatus(window.$clientx.baseDomain);
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `clientx.live_chat_widget`);

    if (IFrameHelper.getBubbleHolder().length) {
      return;
    }
    createBubbleHolder(window.$clientx.hideMessageBubble);
    onLocationChangeListener();

    let className = 'woot-widget-bubble';
    let closeBtnClassName = `woot-elements--${window.$clientx.position} woot-widget-bubble woot--close woot--hide`;
