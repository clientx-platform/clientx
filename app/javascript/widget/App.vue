...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.clientx.ebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.clientx.ebChannel;
    ...
    registerCampaignEvents() {
      ...
      bus.$on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.clientx.ebChannel;
