...
# Description: Install and manage a Clientx installation.
# OS: Ubuntu 20.04 LTS
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/clientx-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Clientx (e.g., clientx.domain.com): ' domain_name
...
serve Clientx at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "clientx.; then
    adduser --disabled-login --gecos "" clientx
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/clientx.config
  file="/opt/clientx.config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/clientx.config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create clientx.db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER clientx.CREATEDB;
    ALTER USER clientx.PASSWORD :'pass';
    ALTER ROLE clientx.SUPERUSER;
...
#   None
##############################################################################
function setup_clientx() {
  local secret=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 63 ; echo '')
  local RAILS_ENV=production
  get_pgpass

  sudo -i -u clientx.<< EOF
  rvm --version
  rvm autolibs disable
  rvm install "ruby-3.2.2"
  rvm use 3.2.2 --default

  git clone https://github.com/clientx-platform/clientx.git
  cd clientx
  git checkout "$BRANCH"
  bundle
  yarn
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=clientx.' .env
...
function run_db_migrations(){
  sudo -i -u clientx.<< EOF
  cd clientx
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:clientx_prepare
EOF
}

##############################################################################
# Setup Clientx systemd services and cwctl CLI
# Globals:
#   None
# Arguments:
#   None
...
function configure_systemd_services() {
  cp /home/clientx/clientx/deployment/clientx-web.1.service /etc/systemd/system/clientx-web.1.service
  cp /home/clientx/clientx/deployment/clientx-worker.1.service /etc/systemd/system/clientx-worker.1.service
  cp /home/clientx/clientx/deployment/clientx.target /etc/systemd/system/clientx.target

  cp /home/clientx/clientx/deployment/clientx./etc/sudoers.d/clientx
  cp /home/clientx/clientx/deployment/setup_20.04.sh /usr/local/bin/cwctl
  chmod +x /usr/local/bin/cwctl

  systemctl enable clientx.target
  systemctl start clientx.target
}
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/clientx-platform/clientx/develop/deployment/nginx_clientx.conf
  cp nginx_clientx.conf /etc/nginx/sites-available/nginx_clientx.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/clientx.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_clientx.conf
  ln -s /etc/nginx/sites-available/nginx_clientx.conf /etc/nginx/sites-enabled/nginx_clientx.conf
  systemctl restart nginx
  sudo -i -u clientx.<< EOF
  cd clientx
  sed -i "s/http:\/\/0.0.0.0:3000/https:\/\/$domain_name/g" .env
EOF
  systemctl restart clientx.target
}
...
function setup_logging() {
  touch /var/log/clientx-setup.log
  LOG_FILE="/var/log/clientx-setup.log"
}
...
***************************************************************************
Woot! Woot!! Clientx server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://clientx.io/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Clientx CLI tool to manage your installation.'
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.clientx.io/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
***************************************************************************
              Clientx Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/clientx-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Clientx?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Clientx. This takes a long while."
  setup_clientx &>> "${LOG_FILE}"
...
https://www.clientx.io/docs/deployment/deploy-clientx-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u clientx.bash -c " cd clientx.&& RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Clientx installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Clientx
  -I                        Install Clientx from a git branch
  -u, --upgrade             Upgrade Clientx to the latest stable version
...
Management:
  ...
  -l, --logs                View logs from Clientx. Supported values include web/worker.
  -r, --restart             Restart Clientx server
...
Report bugs at https://github.com/clientx-platform/clientx/issues
Get help, https://clientx.io/community?utm_source=cwctl

EOF
}

##############################################################################
# Get Clientx web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u clientx-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u clientx-web.1.service -f
  fi
}
...
function upgrade_redis() {
  echo "Upgrading Redis to v7+ for Rails 7 support(Clientx v2.17+)"
  curl -fsSL https://packages.redis.io/gpg | sudo gpg --dearmor -o /usr/share/keyrings/redis-archive-keyring.gpg
  echo "deb [signed-by=/usr/share/keyrings/redis-archive-keyring.gpg] https://packages.redis.io/deb $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/redis.list
  apt update -y
...
function upgrade() {
  get_cw_version
  echo "Upgrading Clientx to v$CW_VERSION"
  sleep 3
  upgrade_prereq
  upgrade_redis
  sudo -i -u clientx.<< "EOF"

  # Navigate to the Clientx directory
  cd clientx
...
EOF
  ...
  # Restart the clientx.server
  systemctl restart clientx.target

}

##############################################################################
# Restart Clientx server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  systemctl restart clientx.target
  systemctl status clientx.target
}
